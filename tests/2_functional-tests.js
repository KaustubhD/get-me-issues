...
suite('Functional Tests', function() {
    ...
    suite('POST /api/issues/{project} => object with issue data', function() {
      ...
      test('Required fields filled in', function(done) {
        ...
        .end(function(err, res){
          ...
          assert.equal(res.body.issue_text, 'Helmet')
      ...
      test('Missing required fields', function(done) {
        ...
        .end(function(err, res){
          ...
          assert.equal(res.body.err, 'Incomplete data')
    ...
    suite('PUT /api/issues/{project} => text', function() {
      ...
      test('Multiple fields to update', function(done) {
        chai.request(server)
        .put('/api/issues/test')
        .send({_id: _id2, issue_text: 'Brand new title', open: false})
        .end(function(err, res){
          assert.equal(res.status, 200)
          assert.equal(res.body.message, 'successfully updated')
          done()
    ...
    suite('DELETE /api/issues/{project} => text', function() {
      ...
      test('No _id', function(done) {
        ...
        .end(function(err, res){
          assert.equal(res.status, 400)
          assert.equal(res.body.err, '_id error')
          done()
        })
      })
      ...
      test('Valid _id', function(done) {
        ...
        .end(function(err, res){
          assert.equal(res.status, 200)
          assert.equal(res.body.message, 'deleted ' + _id1)
          done()
        })
      })
